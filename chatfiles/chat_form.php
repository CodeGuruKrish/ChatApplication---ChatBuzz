...
if($this->chatadd === 1) {
 echo $this->lsite['addnmcd'].': &nbsp; <em id="code_ch">'. substr(md5(date(" j-F-Y, g:i a ")), 3, 4). '</em><br />'.
  ...
  echo '<input class="button" type="button" onclick="exit_chat()" value="Logout" align="left"'.sprintf($this->lsite['logoutchat'], CHATUSER).'</input>'; //to logout before login
  ...
  $user = md5(CHATUSER);
  $sqlflag=mysql_query("UPDATE $tbl_name SET oflag='1' WHERE username= '$user'")or die ('flag query is invalid: ' . mysql_error()); //Update the online flag in database
  $_SESSION['user'] = $user;
}
...
echo "flag set to 0!";
$sqlflag=mysql_query("UPDATE $tbl_name SET oflag='0' WHERE username= '$user'")or die ('flag query is invalid: ' . mysql_error()); //Update the online flag in database
