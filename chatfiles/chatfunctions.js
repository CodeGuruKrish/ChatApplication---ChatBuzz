...
var wsUri = "ws://192.168.0.16:9999";

/*Function to wait for message retrival and send it to the chat window */
  websocket = new WebSocket(wsUri); 
	websocket.onmessage = function(ev) {
		var msg = JSON.parse(ev.data); //PHP sends Json data
		var umsg = msg.message; //message text
		var uname = msg.name; //user name
        var  send_chat = "adchat="+encodeURIComponent(umsg)+"&chatuser="+uname;
	    ajxsend = 1;         // Ajax busy now 
        if (uname == null)
		{
		//Its a null message! Ignore
		}
		else{
...
function addChatS(text) {
  if (chatuserset == 1) {
    ...
    if(chat < 1 || chat > 200) {
	  ...
	  var msg = {
		message: text.adchat.value,
		name: text.chatuser.value
		};
		//convert and send data to server
		//alert("in adchat");
		websocket.send(JSON.stringify(msg));
...
function exit_chat()
{
	...
	var msg = {
		message: document.getElementById('chatuser').value + "left this chat room",
		name: " "
		};
		//convert and send data to server
		websocket.send(JSON.stringify(msg));
